        ...
        try:
            ...
            for entry in feed.entries[:3]:
                if entry.link not in processed:
                    all_articles.append({'title': entry.title, 'link': entry.link, 'source_name': source['name']})
        except: continue
    return all_articles
...
def analyze_and_classify(article):
    ...
    prompt = f"당신은 KOCCA 산업 분석가입니다. 다음 뉴스를 분석해 한국어로 요약하고 [방송·영화·OTT, 게임·융복합, 애니·캐릭터, 만화·웹툰·스토리, 음악, 패션, 통합(정책 등)] 중 하나로 분류하세요.\n\n제목: {article['title']}\n링크: {article['link']}\n\n결과 형식:\nCATEGORY: 카테고리명\nSUMMARY: 한국어 요약\nSUITABLE: YES 또는 NO"
    try:
...
def send_to_jira(article, ai_data):
    ...
    payload = json.dumps({
        "fields": {
            ...
            "labels": [ai_data['cat'].replace("·", "_").replace(" ", "_"), "KOCCA_Bot"],
...
def update_github_markdown(results):
    ...
    content = f"## 📅 {today}\n\n"
    for item in results:
        content += f"* [{item['cat']}] [{item['title']}]({item['link']})\n  * {item['sum']}\n"
...
def main():
    ...
    if results: update_github_markdown(results)
